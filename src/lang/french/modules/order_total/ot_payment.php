...
define(
    ...
    '<h2>Remise et surcharge sur les types de paiement</h2>
    Si vous souhaitez plus d\'échelles de réduction (la valeur par défaut est de <b>3</b>), la valeur de la variable $num (fichiers de langue) ou $this-&gt;num (fichier de module) doit être modifiée à la valeur souhaitée dans tous les fichiers avant l\'installation.
    <hr>
    <h3>Champs d\'échelle de remise</h3>
    <p class="red">Remarque : l\'adresse de livraison est déterminante pour les codes pays. Si un rabais/majoration doit s\'appliquer à tous les pays, utilisez 00 comme code ou omettez complètement la spécification du code pays (y compris le &quot;|&quot; !).</p>
    <h4>Utilisez la notation suivante pour les remises :</h4>
    <pre> <span class="blue">DE|100</span>:<span class="red">4</span>,<span class="green">200</span>:<span class="red">5</span></pre>
    <p>Signification :</p>
    <p>Pour les clients d\'<span class="blue">Allemagne</span>, une remise de <span class="red">4 %</span> est accordée à partir de <span class="green">100 euros</span>, et une remise de <span class="red">5 %</span> à partir de <span class="green">200 euros</span>.</p>
    <pre> <span class="green">100</span>:<span class="red">2</span>,<span class="green">200</span>:<span class="red">3</span></pre>
    <p>Signification :</p>
    <p>Pour les clients de tous les pays, une remise <span class="red">de 2%</span> est accordée à partir de <span class="green">100 euros,</span> à partir de <span class="green">200 euros</span> une remise de <span class="red">3%</span>.</p>
    <h4>Utilisez la notation suivante pour les suppléments :</h4>
    <pre> <span class="blue">DE|100</span>:<span class="red">-3</span></pre>
    <p>Signification :</p>
    <p>Pour les clients d\'<span class="blue">Allemagne</span>, un supplément de <span class="red">3%</span> sera facturé à partir de <span class="green">100 euros</span>.</p>
   <h4>Exemple pour Paypal</h4>
    1. échelle de réduction
    <pre> <span class="blue">DE|0</span>:<span class="red">-1,9&amp;</span><span class="lila">-0,35</span></pre>
    2ème barème de remise
    <pre> <span class="blue">00|0</span>:<span class="red">-3.4&amp;</span><span class="lila">-0.35</span></pre>
    <p>Signification :</p>
    <p>Pour les clients d\'<span class="blue">Allemagne</span>, une surcharge de <span class="red">1,9%</span> plus 0 <span class="lila">,35</span> euros s\'applique à partir de <span class="lila">0 euros</span> (c\'est-à-dire toujours).</p>
    <p>Pour les clients de <span class="blue">tous les autres pays (00=tous</span> ), un supplément de <span class="red">3,4 %</span> plus 0 <span class="lila">,35</span> euro sera facturé à partir de <span class="green">0 euro</span>.</p>
    <p>Il est important que les entrées soient effectuées dans le bon ordre (les autres pays toujours en dernier) et que &quot;Calcul multiple&quot; soit réglé sur &quot;faux&quot;, sinon les deux suppléments seront calculés.</p>
    <h4>Exemple pour les montants fixes</h4>
    <pre> <span class="green">0</span>:<span class="red">0&amp;</span><span class="lila">-2</span></pre>
    <p>Signification :</p>
    <p>Pour les clients de tous les pays, une surcharge de <span class="red">0%</span> (c\'est-à-dire aucune surcharge en pourcentage) plus <span class="lila">2,00 EUR</span> (la surcharge fixe) est calculée à partir de <span class="lila">0 EUR</span> (c\'est-à-dire toujours).</p>
    <hr>
    <h3>Champs de type de paiement</h3>
    <p>Saisissez le <b>code interne</b> du mode de paiement dans les champs, par exemple le <b>mandat postal</b> pour le paiement à l\'avance ou le <b>code</b> pour le contre-remboursement.</p> <p>Séparez les différents types de paiement par des virgulesVoir</p>Modules -&gt; Options de paiement -&gt; <p>colonne</p>&quot;Nom du module (pour usage interne)&quot;.<br/><br/>
    <hr>
    <h3>Affichage du type de paiement dans le processus de commande</h3>
    Si vous souhaitez que la remise correspondante soit affichée dans le processus de commande lors de la sélection du mode de paiement, réglez l\'option &quot;Afficher dans les modes de paiement&quot; sur &quot;true&quot;. <br/><br/>
    En outre, vous pouvez définir le type d\'affichage avec l\'option &quot;Type d\'affichage dans le processus de commande lors de la sélection du paiement&quot; :
    <p> -- Par défaut : pourcentage ou montant, selon les entrées du barème de remise.</p>
    <p> -- Prix : Le montant réel est toujours affiché.</p></hr></hr></hr>'
);
...
if (defined('MODULE_ORDER_TOTAL_PAYMENT_NUMBER')) {
    for ($j = 1; $j <= MODULE_ORDER_TOTAL_PAYMENT_NUMBER; $j++) {
        define('MODULE_ORDER_TOTAL_PAYMENT_PERCENTAGE' . $j . '_TITLE', '<hr>' . $j . '. Barème des rabais');
        ...
        define('MODULE_ORDER_TOTAL_PAYMENT_TYPE' . $j . '_DESC', 'Formes de paiement sur lesquelles la remise sera appliquée');
    }
}
